  ...
  Future<void> _updatePassword() async {
    try {
      ...
      for (int i = 0; i < lines.length; i++) {
        if (lines[i].startsWith('Name:') && lines[i].contains(_name!) &&
            lines[i + 1].startsWith('Email:') && lines[i + 1].contains(_email!) &&
            lines[i + 2].startsWith('Phone Number:') && lines[i + 2].contains(_phone!)) {
          userBlock = lines[i];
          break;
        }
      }
      ...
      if (userBlock != null) {
        for (int i = 0; i < lines.length; i++) {
          if (lines[i].startsWith('Password:') && lines[i - 3] == userBlock) { 
